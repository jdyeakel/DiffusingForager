...
The NSM describes a broad range of dynamics, yet organisms are likely unable to access most of the total parameter space. Here we use allometric scaling relationships to constrain the covariation of rates in a principled and biologically meaningful manner. % we have not described the realistic regimes occupied by organisms where the
...
\begin{eqnarray}
m\left(t\right)=\left(\frac{B_{0}}{B_{m}}\right)^{1/(\eta-1)}\left[1-\left(1-\frac{B_{m}}{B_{0}}m_{0}^{1-\eta}\right)e^{-b\left(1-\eta\right)t}\right]^{1/\left(1-\eta\right)}%\left(\frac{a}{b}\right)^{1/\left(1-\eta\right)}
\end{eqnarray}
