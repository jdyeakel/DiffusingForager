...
reduced-size hungry state where reproduction is impossible.  This transition
...
and assuming that once this mass is fully digested the organism begins to
starve, this would imply that $\epsilon=1-f_{0}M^{\gamma}/M$.  Using this
criterion in Eq.~\ref{mt}, the time scale for starvation is given by
\begin{equation}
\label{eq:sigma}
...
to find the timescale to return to the replete-state mass from a given
reduced starvation mass.  From the solution to Eq.~\ref{balance}
\begin{eqnarray}
